      ...
      re = /[^\s።፤፣•፡፥\?!”“\u2039\u203A()-/]+[^\s።፤፣•፡፥\?!”“()\d\u2039\u203A]*[^\s።፤፣•፡፥\?!”“\u2039\u203A()-/\d]+/g,
		...
		/*
			...
			*/
			...
			var current_document_word_frequency = JSON.parse(JSON.stringify(tf)), sentence_word_length = [];
			...
			for (var j = sentences.length - 1; j >= 0; j--) {
				...
				if(sentence_words != null) {
					sentence_word_length[j] = sentence_words.length
...
// Rank all the words
  for (var word in tf) {
   ...
   // word_rank = word_rank.slice(0,150)
			...
			summary_size = Math.round(summary_size/100 * total_words)
			summary_words = 0
			summary_sentences = 0
			...
			for (var i = 0; i < summary_size; i++) {
				...
				summary_sentences++
				summary_words = summary_words + sentence_word_length[score_sentences[sorted[i]]]
				if(summary_words >= summary_size)
					break;
			}
			...
			var output = {
				...
				'sentence_word_length': sentence_word_length,
				'clusters': clusters,
				'sentences': sentences,
